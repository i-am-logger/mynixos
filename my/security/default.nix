...
in
{
  config = mkMerge [
    ...
    (mkIf (cfg.enable && cfg.sshAgentSudo) {
      assertions = [
        {
          assertion = config.services.openssh.enable;
          message = ''
            my.security.sshAgentSudo needs /etc/ssh/authorized_keys.d/<user>,
            which nixpkgs writes only while services.openssh.enable is true.
            Without sshd this host trusts an absent file: every sudo falls
            through to the security key, which on a headless host is nobody.
          '';
        }
      ];
      ...
      security.pam = {
        sshAgentAuth = {
          enable = true;
          # Root-owned 0444, generated by my/network/openssh from each user's
          # yubikeys[].sshPublicKey. Deliberately not ~/.ssh/authorized_keys —
          # see options.nix.
