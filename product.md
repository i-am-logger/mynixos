...
- **`my.dev`** -- Development infrastructure. `my.dev.enable` is auto-derived from user `dev.enable`, and `my.dev.containers` carries the settings that belong to the machine rather than an account: `backend`, an enum of `podman` (the Linux default) and `docker`, plus `autoStart` for the darwin Colima launchd agent. The two backends cannot coexist -- podman's `dockerCompat` and `virtualisation.docker` both claim `/run/docker.sock` and the `docker` binary -- which is why it is an enum rather than two switches. Whether containers are installed for a person is the per-user `my.users.<name>.dev.containers.enable`, and that one option reads the same on both platforms: rootless podman on NixOS, Colima on macOS, where the default flips to `docker` because Colima serves no other backend.
